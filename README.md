...
1. В директории должно быть два обязательных файла — `hosts.json, OWNERS.yaml, style.css`. 
    * В `hosts.json` должен быть массив доменов для которых должны применяться текущие стили, например `https://www.example.com, https://m.example.com`. Протокол (http, https) обязателен.
    * `OWNERS.yaml` должен содержать список логинов на https://github.com кому разрешено править стили для текущего домена. 
